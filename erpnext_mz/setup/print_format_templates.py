...
"""
    ...
    def get_common_header_macro(self, document_title):
        """Common header macro for all documents"""
        return """
{%- macro add_header(page_num, max_pages, doc, letter_head, no_letterhead, footer, print_settings=None, print_heading_template=None) -%}
    ...
    <div class="document-header">
        <div class="row">
            <div class="col-xs-8">
                <h1 class="document-title">{{ _(""" + document_title + """) }}</h1>
            </div>
            <div class="col-xs-4 text-right">
                ...
                <div class="document-date">
                    {{ frappe.utils.format_date(doc.posting_date or doc.transaction_date or doc.creation) }}
                </div>
            </div>
        </div>
    </div>
        ...
        if custom_columns is None:
            custom_columns = [
                ...
                ("Descrição", "text-left", """
                    ...
                    {% endif %}
                """),
                ("Qtd", "text-center", "{{ item.get_formatted('qty', 0) }}"),
                ("U.M.", "text-center", "{{ item.get_formatted('uom', 0) }}"),
        ...
        for field, label in totals_fields:
            if field == "grand_total":
                ...
                <div class="totals-row grand-total">
                    <span class="totals-label">{{ _("""" + label + """") }}:</span>
