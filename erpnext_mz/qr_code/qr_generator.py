...
def create_validation_data(doc):
    ...
    validation_data = {
        ...
        "company": doc.company,
        "date": document_date,
        "amount": float(doc.grand_total) if hasattr(doc, 'grand_total') else float(doc.total),
        "currency": doc.currency if hasattr(doc, 'currency') else company.default_currency,
