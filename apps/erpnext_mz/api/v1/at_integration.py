...
def transmit_invoice(invoice_name):
    ...
    try:
        # Validate inputs
        if not invoice_name:
            return build_response("error", "Nome da fatura é obrigatório")
        
        # Check if invoice exists
        if not frappe.db.exists("Sales Invoice", invoice_name):
            return build_response("error", "Fatura não encontrada")
        
        # Check permissions
        invoice = frappe.get_doc("Sales Invoice", invoice_name)
        if not frappe.has_permission("Sales Invoice", doc=invoice):
            return build_response("error", "Permissões insuficientes")
        ...
        try:
            ...
            if existing:
                return build_response("success", "Já transmitido", {"invoice": invoice_name})
        except Exception:
            pass
        ...
        if result.get("success"):
            return build_response("success", "Fatura transmitida com sucesso", result)
        else:
            return build_response("error", f"Transmissão falhou: {result.get('message')}", result)
        
    except Exception as e:
        frappe.log_error(f"AT integration API failed: {str(e)}")
        return build_response("error", f"Transmissão falhou: {str(e)}")
...
def transmit_saf_t(company, period, file_type):
    ...
    try:
        ...
        if not company or not period or not file_type:
            return build_response("error", "Empresa, período e tipo de ficheiro são obrigatórios")
        
        if file_type not in ['sales', 'payroll']:
            return build_response("error", "O tipo deve ser 'sales' (vendas) ou 'payroll' (folha)")
        ...
        if not files:
            return build_response("error", f"Ficheiro SAF-T {file_type} não encontrado para o período {period}")
        ...
        try:
            ...
            if path.startswith("/files/"):
                ...
                try:
                    ...
                    xml_content = None
        if not xml_content:
            return build_response("error", "Não foi possível ler o conteúdo do ficheiro SAF-T")
        ...
        if result.get("success"):
            return build_response("success", f"SAF-T {file_type} transmitido com sucesso", result)
        else:
            return build_response("error", f"Transmissão falhou: {result.get('message')}", result)
...
def get_transmission_status(company, period=None):
    ...
    try:
        # Validate inputs
        if not company:
            return build_response("error", "Empresa é obrigatória")
        ...
        return build_response("success", "Estado obtido com sucesso", {"logs": logs})
        
    except Exception as e:
        frappe.log_error(f"AT status API failed: {str(e)}")
        return build_response("error", f"Falha ao obter estado: {str(e)}")
...
def configure_at_integration(company, api_endpoint, api_key, enabled=True):
    ...
    try:
        ...
        if not company or not api_endpoint or not api_key:
            return build_response("error", "Empresa, URL do API e chave são obrigatórias")
        ...
        return build_response("success", "Integração com AT configurada com sucesso")
        
    except Exception as e:
        frappe.log_error(f"AT configuration API failed: {str(e)}")
        return build_response("error", f"A configuração falhou: {str(e)}")
